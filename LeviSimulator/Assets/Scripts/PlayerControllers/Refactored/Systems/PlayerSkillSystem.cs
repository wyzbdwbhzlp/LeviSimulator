...
namespace PlayerControllers.Refactored.Systems
{
    ...
    {
        [Title("运行中数据")] 
        [ReadOnly] [SerializeField][LabelText("冲刺冷却计时")] private float dashCoolDownTimer;
        [ReadOnly] [SerializeField][LabelText("当前冲刺数量")] private int currentDashCount;
        [ReadOnly] [SerializeField][LabelText("子弹时间计时")] private float bulletTimeEnergy;
...
private Vector3 CalculateDashVelocity()
{
    ...
    Vector3 blendedRight = GetBlendedDirection(cameraRight);

    // 计算目标冲刺方向
    Vector3 targetDirection = CalculateTargetDirection(moveInput, blendedForward, blendedRight);
...
/// 根据输入计算目标冲刺方向
/// </summary>
private Vector3 CalculateTargetDirection(Vector2 moveInput, Vector3 blendedForward, Vector3 blendedRight)
{
    Vector3 targetDirection;
    
    // 检查是否有有效的移动输入
    if (moveInput.sqrMagnitude > _skillConfig.MoveInputThreshold)
    {
        // 基于输入计算方向
        targetDirection = blendedForward * moveInput.y + blendedRight * moveInput.x;
        
        if (targetDirection.sqrMagnitude > 0.0001f)
        {
            targetDirection.Normalize();
            _lastNonZeroDashDirection = targetDirection;
        }
        else
        {
            // 输入导致的零向量，使用摄像机前方
            targetDirection = blendedForward;
        }
    }
    else
    {
